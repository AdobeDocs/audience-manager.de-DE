...
Mit diesen Tools können Sie Datenschutzanfragen von Verbrauchern senden, die unter [!DNL GDPR] und [!DNL CCPA] gestellt wurden.
...
* Über die **[!DNL Privacy Service API]**. Die Dokumentation [hier](https://experienceleague.adobe.com/docs/experience-platform/privacy/api/overview.html?lang=en) und die [!DNL API] Referenz [hier](https://developer.adobe.com/experience-platform-apis/references/privacy-service/) finden Sie in der Dokumentation.

Beim Senden individueller Datenschutzanfragen können Sie beliebige [!DNL Audience Manager]-IDs (IDs), wie im Abschnitt **[Audience Manager-IDs](data-privacy-ids.md)** beschrieben, zusammen mit den entsprechenden Namespace-IDs (Datenquellen-IDs) senden.
...
Adobe ist sich Ihrer Verpflichtung bewusst, Datenschutzanfragen von Kunden innerhalb von 30 Tagen zu erfüllen. Aus diesem Grund ist [!DNL Adobe] verpflichtet, Ihre Datenlöschanfrage so bald wie möglich zu verarbeiten.

Als Antwort auf Ihre Anfragen zum Löschen von Verbraucherdaten löscht [!DNL Audience Manager] Eigenschaften und Segmente, die mit der in der Anfrage enthaltenen [!DNL Audience Manager] -Kennung verknüpft sind. Darüber hinaus werden die entsprechenden [!DNL Audience Manager]-IDs für die Person, die sich von der weiteren Datenerfassung durch [!DNL Audience Manager] abgemeldet hat und die entsprechenden ID-Zuordnungen entfernt.

Wenn Sie deklarierte IDs, z. B. geräteübergreifende [!DNL CRM] IDs oder [!DNL cookie] IDs, in Datenschutzanfragen senden, führt [!DNL Audience Manager] den erforderlichen Löschvorgang auf allen verknüpften Geräten durch (bis zu 100 Geräte pro deklarierter ID).

[!DNL Audience Manager] versucht, Aktivierungspartner über Löschanfragen zu benachrichtigen, indem es ihnen Informationen zur Aufhebung der Segmentierung für betroffene Personen sendet, die das Löschen bestimmter Daten anfordern. Allerdings können einige Aktivierungspartner:
...
2. keine Aktualisierungen von [!DNL Audience Manager] mit einer Häufigkeit von weniger als 30 Tagen erhalten. In diesen Fällen können [!DNL Audience Manager] -Kunden Löschanfragen nicht automatisiert bis [!DNL Audience Manager] an Aktivierungspartner senden.

In diesen Fällen können Sie Löschanfragen nicht automatisiert durch [!DNL Audience Manager] an Aktivierungspartner senden.

In der Dokumentation zur Liste der [gerätebasierten Ziele](assets/AAM-Partners-October2019.xlsx) erfahren Sie, welche [!DNL Audience Manager] Aktivierungspartner die Aufhebung der Segmentierung unterstützen.

## Opt-out-Anfragen {#opt-out-requests}

[!DNL Audience Manager] unterstützt branchenweite Standards für die Opt-out-Verwaltung. Hier finden Sie vollständige Informationen zu den von [!DNL Audience Manager] unterstützten Opt-out-Typen.
...
Das globale Opt-out stellt eine Opt-out-Option für alle [!DNL Audience Manager]- und anderen [!DNL Adobe Experience Cloud]-Lösungen für alle Marken dar. In der folgenden Tabelle werden die Methoden für das globalen Opt-out-Verfahren angezeigt:
...
* [!DNL Audience Manager] beendet die Datenerfassung, Segmentierung oder Aktivierung für die [!DNL CRM] ID und die letzte Geräte-ID, die mit der [!DNL CRM] -ID verknüpft ist.
...
* [!DNL Audience Manager] beendet die Datenerfassung, Segmentierung oder Aktivierung für den Partner in Zukunft für die Geräte-ID.
...
Um Sie bei der Automatisierung Ihrer Datenschutzanfragen für Verbraucher zu unterstützen, versucht [!DNL Audience Manager], Aktivierungspartner über Löschanfragen von betroffenen Personen zu informieren, indem diese Informationen zur Aufhebung der Segmentierung (oder zum Entfernen der Segmentinformationen) senden.
...
2. Sie können keine Aktualisierungen von [!DNL Audience Manager] häufiger als einmal in 30 Tagen erhalten.
...
Da [!DNL Audience Manager] nicht die Quelle der Daten ist, gibt es eine begrenzte Rolle für die Datenkorrektur in [!DNL Audience Manager]. Die Korrektur könnte bedeuten, dass der Verbraucher angefordert hat, entweder von einem falschen [!UICONTROL trait]/[!UICONTROL segment] disqualifiziert oder auf den gewünschten [!UICONTROL trait]/[!UICONTROL segment] qualifiziert zu werden.

[!DNL Audience Manager] -Kunden können die relevanten Signale/Eigenschaften/Segmente anhand von Benutzerprofilen erfassen und diese Informationen über die [Offline-Datenerfassung](../../integration/sending-audience-data/batch-data-transfer-explained/batch-data-transfer-overview.md) an [!DNL Audience Manager] senden. Beachten Sie, dass der Benutzer weiterhin für die ursprünglichen [!UICONTROL trait] und [!UICONTROL segments] qualifiziert ist, wenn er sein Verhalten wiederholt.
