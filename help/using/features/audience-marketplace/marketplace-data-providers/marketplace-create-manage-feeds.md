...
1. Geben Sie eine kurze Beschreibung ein (maximal 255 Zeichen).
   ...
   Eine gute Beschreibung sollte Ihren Feed genau beschreiben. Sie können beispielsweise Text für Marketing-Kategorien, demografische Daten und die geografische Abdeckung (z. B. [!DNL US] oder Nordamerika) einbeziehen. Der Beschreibungstext ist durchsuchbar und hilft Käufern, Ihren Feed zu finden oder zu bewerten. Eine gute Beschreibung ist ein wichtiger Teil der Attraktivität für Abonnenten Ihres Daten-Feeds.
...
1. Wählen Sie in [!UICONTROL Plan Types] die Optionen aus, die Sie verwenden möchten, und klicken Sie auf **[!UICONTROL Add Plan]**.

   Feeds können mehrere Pläne enthalten. Pläne können mehrere Anwendungsfälle enthalten. Weitere Informationen finden Sie unter [Planen von Typen für Daten-Feeds](../../../features/audience-marketplace/marketplace-data-providers/marketplace-create-manage-feeds.md#plan-types).

1. Klicken Sie auf **[!UICONTROL Save]** , um Ihren Daten-Feed *zu speichern, ohne* ihn zu aktivieren.
...
* **[!UICONTROL Private]** und **[!UICONTROL Branded]**: Die Liste [!UICONTROL Marketplace] des Käufers zeigt den Namen des Verkäufers in der Spalte &quot;Anbieter&quot;an und alle anderen Daten werden ausgeblendet.

* **[!UICONTROL Private]** und **[!UICONTROL Unbranded]**: Die Liste [!UICONTROL Marketplace] des Käufers enthält nur den Daten-Feed-Namen und die Beschreibung. Der Name des Datenanbieters wird als [!UICONTROL Private Seller] angezeigt.

Informationen dazu, wie private Feeds für Käufer aussehen, finden Sie im Abschnitt &quot;Käufer&quot;unter [Private Daten-Feeds](../../../features/audience-marketplace/marketplace-private-feeds.md).
...
1. Klicken Sie im Pop [!UICONTROL Confirmation] auf **[!UICONTROL Yes]** , um ein Abonnement zu deaktivieren, oder auf **[!UICONTROL Cancel]** , um es zu beenden, ohne Abonnementänderungen vorzunehmen.
...
[!DNL Plan types] sind wesentliche Komponenten in einem [!UICONTROL Audience Marketplace] -Daten-Feed. Als Datenanbieter können Sie damit mehrere Anwendungsfälle und Preisoptionen für Ihre Feeds erstellen. Darüber hinaus kann es eine gute Strategie sein, für jeden Daten-Feed einige Pläne zu erstellen. Dadurch erhalten Käufer verschiedene Optionen, unter denen sie nach Daten suchen, die sie modellieren oder an ein Ziel senden können.

[Erstellen Sie einen Daten-Feed](../../../features/audience-marketplace/marketplace-data-providers/marketplace-create-manage-feeds.md#create-public-private-data-feed), um [!UICONTROL Plan Types] auszuwählen.
...
Ein **[!UICONTROL Segments and Overlap]** -Anwendungsfall erstellt einen Plan, der es Käufern ermöglicht, Eigenschaftsdaten in einem [Überschneidungsbericht zwischen Eigenschaften](../../../reporting/dynamic-reports/trait-trait-overlap-report.md#trait-to-trait-overlap-report) zu vergleichen. Darüber hinaus können Käufer Ihre Daten zu Segmenten hinzufügen und Vergleiche mit den Berichten [Segment-zu-Merkmal](../../../reporting/dynamic-reports/segment-trait-overlap-report.md) und [Segment-zu-Segment](../../../reporting/dynamic-reports/segment-segment-overlap-report.md) anstellen.

Jeder Daten-Feed muss mindestens einen [!UICONTROL Segments and Overlap] -Anwendungsfall enthalten. Käufer können keine anderen Pläne in einem Daten-Feed abonnieren, wenn der Feed keinen [!UICONTROL Segments and Overlap] -Anwendungsfall enthält, weder allein noch in Kombination mit einem anderen Anwendungsfall.

Überschneidungen können Käufern helfen,

* **Erweiterung der Zielgruppenreichweite:** Geringe Überschneidungen deuten darauf hin, dass Ihre Eigenschaften Benutzer enthalten, die der Käufer noch nicht gesehen hat. Daher möchten Käufer möglicherweise, dass diese Eigenschaften ihren Zielgruppensegmenten neue Benutzer hinzufügen.
* **Vorhandene Zielgruppen verbessern:** Eine hohe Überschneidung lässt darauf schließen, dass Ihre Eigenschaften Benutzer enthalten, die denen ähneln, von denen ein Käufer bereits weiß. Daher möchten Käufer möglicherweise, dass diese Eigenschaften dazu beitragen, gezielte, inkrementelle Verbesserungen an entwickelten Zielgruppen vorzunehmen.
...
Ein **[!UICONTROL Modeling]** -Anwendungsfall erstellt einen Plan, der es Käufern ermöglicht, ihre Eigenschaften mit ihrer [algorithmischen Modellierung](../../../features/algorithmic-models/understanding-models.md#understanding-models) zu vergleichen. Käufer betrachten die Modellergebnisse, um neue Zielgruppen in Ihren Daten zu finden, die ähnliche Konversionsattribute wie ihre eigenen aufweisen. Geben Sie diesen Anwendungsfall wie folgt an:
...
<table id="table_CCEAAF24295942EA82F20753827D1A23"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <b><span class="uicontrol"> Monatlich in Rückständen</span></b> ist die einzige Option. Der Abrechnungszyklus endet am 10. Tag jedes Monats. </td> 
  </tr> 
  <tr> 
...
Um Käuferanfragen zu überprüfen, zu genehmigen oder abzulehnen, gehen Sie zu [!UICONTROL My Shared Data] und:
...
Mit Rabatten in [!UICONTROL Audience Marketplace] können Sie den veröffentlichten Preis eines Daten-Feeds für einzelne Abonnenten reduzieren. Sie können Abonnenten, die eine Abonnementanforderung gesendet haben, oder Abonnenten, die Details zu einem Daten-Feed angefordert haben, Rabatte anbieten. Rabatte gelten für Feeds mit einem Pauschalsatz von [!DNL CPM]. Rabatte können hilfreich sein, wenn Sie Abonnementanreize für neue Kunden bereitstellen oder die Kundentreue belohnen möchten.

## Anwenden von Rabatten auf einen Daten-Feed {#apply-discounts}

<!-- marketplace-seller-discounts.xml -->

Um einen Feed zu diskontieren, fügen Sie dem Rabattfeld einen Rabattbetrag in % hinzu und bestätigen Sie Ihre Änderungen. Datenanbieter können einen Datenfeed in [!UICONTROL Audience Marketplace] von einem der folgenden Rabatte abziehen:

* **[!UICONTROL My Shared Data > Potential Subscribers]**
* **[!UICONTROL My Shared Data > Details Requests]**

In diesen Beispielen hat der Verkäufer dem [!UICONTROL Software Audience] -Daten-Feed einen Rabatt von 10 % hinzugefügt.
