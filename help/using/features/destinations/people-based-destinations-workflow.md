...
Die Konfiguration von [!DNL People-Based Destinations] führt Sie durch mehrere Abschnitte des Audience Managers und erfordert verschiedene Einstellungen und Methoden zum Daten-Onboarding, je nachdem, welche Art von Kundendaten Sie bereits im Audience Manager haben und welche Art von Zielgruppen-Targeting Sie durchführen möchten.
...
Ihr Unternehmen, eine Fluggesellschaft, verfügt über verschiedene Kundenstufen (Bronze, Silber und Gold), und Sie möchten jeder dieser Stufen über soziale Plattformen personalisierte Angebote unterbreiten. Sie verwenden Audience Manager zur Analyse der Kundenaktivität auf Ihrer Website. Allerdings verwenden nicht alle Kunden die Mobile App der Fluggesellschaft, und einige von ihnen haben sich nicht auf der Website des Unternehmens angemeldet. Ihre Kundendaten sind größtenteils auf Mitgliedschafts-IDs und E-Mail-Adressen beschränkt.

Um sie über Social Media und ähnliche personenbasierte Kanäle anzusprechen, können Sie Ihre [gehashten E-Mail-](people-based-destinations-prerequisites.md)) in den Audience Manager einbinden und sie mit Ihren bestehenden Online-Aktivitätseigenschaften kombinieren, um neue Zielgruppensegmente zu erstellen. Als Nächstes können Sie diese Segmente verwenden, um Ihre Zielgruppe mithilfe von [!DNL People-Based Destinations] anzusprechen.

**B) Zielgruppen-Targeting ausschließlich auf der Grundlage Ihrer Offline-Benutzeraktivität**. In diesem Szenario enthält Ihr [!DNL CRM] Ihre E-Mail-Adressen und andere Kundenattribute, aber Kunden haben überhaupt nicht mit Ihrer Website interagiert, sodass Sie im Audience Manager keine Kundenaktivität haben. Hier ist ein Beispiel, das dieses Szenario veranschaulicht:
...
Zur Zielgruppenbestimmung in [!DNL People-Based Destinations] müssen Sie [SHA256-Hash](people-based-destinations-prerequisites.md)-Versionen Ihrer Kunden-E-Mail-Adressen senden. Je nach vorhandener Audience Manager-Konfiguration kann eines der beiden folgenden Szenarien vorliegen:
...
Je nach dem Typ der Kunden-IDs, die Sie im Audience Manager haben (siehe [3. Identifizieren Sie den Typ der von Ihnen verwendeten Kunden-IDs (CRM](people-based-destinations-workflow.md#identify-customer-id)IDs). Sie befinden sich in einem der folgenden Szenarien:
...
* Verwenden Sie [deklarierte IDs](../declared-ids.md), um Ihre gehashten E-Mail-Adressen zu deklarieren, wenn Sie authentifizierte Kunden-IDs übergeben. Wenn Sie diese Methode verwenden, richtet sich der Audience Manager in Ihrem Namen nur an Ihre Hash-E-Mail-Adressen von Benutzern, die sich online authentifiziert haben. Die E-Mail-Adressen, die in personenbasierten Kanälen angesprochen werden, sind nur diejenigen in den deklarierten ID-Ereignisaufrufen. Andere mit der Kunden-ID verknüpfte E-Mail-Adressen werden nicht in Echtzeit aktiviert.
