...
* **Verwenden Sie die dateibasierte ID-Synchronisierung**. Einzelheiten dazu, wie ID-Synchronisierungsdateien aussehen sollten, finden Sie unter [Name und Inhaltsanforderungen für ID-Synchronisierungsdateien](../integration/sending-audience-data/batch-data-transfer-explained/id-sync-file-based.md) . Bei Verwendung dieser Methode können Sie alle Hash-E-Mail-Adressen aus Ihrer [!DNL CRM] Datenbank Zielgruppe haben.
...
Die Offline-Datenerfassung unterstützt Anwendungsfälle ohne Authentifizierung. Eine neue Profil Merge-Regel ([!UICONTROL All Cross-Device Profiles]), die auf allen Offline- [!DNL CRM] Profilen unabhängig von der Authentifizierung ausgeführt wird, ist als Teil [!DNL People-Based Destinations]der Funktion verfügbar. Diese Methode soll die Erfassung authentifizierter Audiencen aus Online-Quellen ergänzen.
...
Audience Manager nimmt die unverarbeitete E-Mail-Adresse nicht auf und wir können nicht bestätigen, dass der Hash korrekt ausgeführt wurde. Weitere Informationen dazu, wie Sie die Daten mit [Daten im Internet hash sollten, finden Sie unter Voraussetzungen und Überlegungen](../features/destinations/people-based-destinations-prerequisites.md) .

## Regeln zum Zusammenführen von Profilen {#profile-merge-rules}

**Gibt es eine neue Profil Merge-Regel, die ich verwenden kann[!DNL People-Based Destinations]?**

Ja. Kunden, die einen Kauf tätigen, [!DNL People-Based Destinations] erhalten außerdem Zugriff auf eine neue Profil-Mergeregel für die Offline-Segmentierung. Die Regel wird aufgerufen [!DNL All Cross-Device Profiles] und für die ausschließliche Offline-Segmentierung verwendet. Wenn Sie sich für anmelden, [!DNL People-Based Destinations]ist dies die vierte Profil-Zusammenführungsregel, die Sie erstellen können, abgesehen von den drei vorhandenen authentifizierungsbasierten Regeln.

**Muss ich meine bestehenden Audiencen-Segmente Duplikat haben, um sie zu aktivieren[!DNL People-Based Destinations]?**

Es hängt von Ihrem Anwendungsfall ab. Wenn Sie planen, vorhandene Erstanbietersegmente in benutzerbasierten Kanälen zu aktivieren, müssen Sie keine neuen Segmente erstellen. Sie können die Segmente einfach einem benutzerbasierten Ziel zuordnen.

Wenn Sie planen, neue Offline-Audiencen in benutzerbasierten Kanälen zu aktivieren, müssen Sie neue Erstanbietersegmente mithilfe der [!DNL All Cross-Device Profiles] Zusammenführungsregel erstellen.
>[!NOTE]
>
...
**Haben Sie[!DNL People-Based Destinations]Einblicke in Übereinstimmungsraten oder adressierbare Audiencen?**

Nein. Beim Senden von Segmenten an [!DNL People-Based Destinations]die Audience erfolgt die Zuordnung der Audience auf der Partnerseite. Adobe hat keinen Zugriff auf diese Metriken. Audience Manager zeigt Ihnen die maximale freigegebene Audience für jedes Ziel und die Echtzeitanzahl der einem Segment angehörenden Personen. Diese Informationen können Ihnen bei der Planung und Prognose von Kampagnen helfen.

**Wie würden Raten theoretisch mit anderen Methoden zum Senden von Audiencen an Zielplattformen verglichen[!DNL People-Based Destinations]werden?**
...
Audience Manager können bis zu 10 E-Mails pro Benutzer sammeln und an Zielplattformen senden. Die E-Mail-Adressen müssen jedoch über Synchronisierungsdateien erfasst werden. Nachdem Audience Manager die E-Mail-Adressen an die Zielplattformen gesendet hat, müssen die Plattformen die E-Mail-Adressen mit der eigenen Benutzerbasis abgleichen. Einige Plattformen verfügen möglicherweise über zusätzliche Diagramme für E-Mail-Adressen, die von Audience Manager an Benutzer-Profil gesendet werden.
...
**Was muss ich tun, bevor ich Audiencen-Segmente an senden kann[!DNL Facebook]?**

Bevor Sie Audiencen [!DNL People-Based Destinations] [!DNL Facebook]an senden können, müssen Sie die folgenden Aufgaben ausführen:

1. Hinzufügen das Adobe Experience Cloud-Geschäftskonto als Werbepartner in Ihrem [!DNL Facebook Ad Account]. Verwenden Sie `business ID=206617933627973`. Weitere Informationen finden Sie unter Hinzufügen Partner für Ihren Business Manager.
...
**Unterstützt[!DNL People-Based Destinations]das Targeting von Audiencen in anderen[!DNL Facebook]Apps, z. B.[!DNL Instagram]?**

Sie können Apps [!DNL People-Based Destinations] aus [!DNL Facebook]der gesamten Familie verwenden, die von [!DNL Custom Audiences], einschließlich [!DNL Facebook], [!DNL Instagram]und [!DNL Audience Network] [!DNL Messenger]unterstützt werden. Die Auswahl der App, für die Sie die Kampagne ausführen möchten, wird auf der Platzierungsebene in angezeigt [!DNL Facebook Ads Manager].

**Was ist der Unterschied zwischen[!DNL People-Based Destinations]und[!DNL Website Custom Audiences]?**

[!DNL People-Based Destinations] nutzt die [!DNL Custom Audiences (CA)] Integration mit [!DNL Facebook]. Der Unterschied zwischen [!DNL WCA] und [!DNL CA] Integrationen ist der Schlüssel, den Kunden beim Senden von Audiencen an verwenden [!DNL Facebook]. [!DNL WCA] verwendet das [!DNL Facebook] Pixel (d. h. eine Website-Benutzer-ID), während [!DNL People-Based Destinations] Hash-E-Mail-Adressen zur Integration in verwendet werden [!DNL CA].

Sie können die [!DNL Facebook][!DNL WCA] Integration von Audience Manager über die [!DNL URL Destinations] Funktion kostenlos nutzen.

Diese beiden Integrationen ergänzen sich. Sie können beide verwenden, um eine bessere Abdeckung der Audience sicherzustellen. Sie [!DNL WCA] können beispielsweise zum Aufsuchen verwendet werden, wenn eine Firma nach Zielgruppe-Website-Besuchern sucht, die kein Konto registriert haben, während [!DNL People-Based Destinations] Sie bei der Zielgruppe bestehender Kunden helfen können, die ihre E-Mail-Adresse angegeben haben, aber möglicherweise nicht die Website besucht haben.
